...
The following code limits the data to only when a station is without vending.
...
In order to find the proportion of entrances without vending, the numerator would be `r sum(nycsub_filt$entry == "TRUE")` and denominator would be `r nrow(nycsub_filt)`. The proportion of entrances/exits without vending that allow entry is `r 63/183*100`%.
...
The following code chunk reads and cleans the National Politicians dataset and also changes the month number into a month name.
...
The next code chunk reads and cleans the S&P dataset and also changes the month number into a month name.
...
The following code chunk reads and tidies the Unemployment dataset and also changes the month abbreviations into month names.
...
The next bit of code joins the above three datasets together by year and month to form the final dataset.
